...
public:
    ...
    static const flashpart_t *getMap( eMcuInternal & mcu ) {
        for ( uint32_t i = 0; i < mcu_int_dids.x8parts.count; i++ ) {
            if ( mcu_int_dids.x8parts.parts[ i ].did == mcu &&
                 mcu_int_dids.x8parts.parts[ i ].count > 0 &&
                 mcu_int_dids.x8parts.parts[ i ].partitions != nullptr )
                return &mcu_int_dids.x8parts.parts[ i ];
        }
